...
fn-whatis = @ {
    let (result = ) {
        for (i = $*) {
                ...
                # TODO: Fix stderr redirection when implementing file descriptors with arrows
                echo $message
                result = $result 1
            } {
                echo ${%whatis $i}
...
fn-%one = @ {
    if {!~ $#* 1} {
        if {~ $#* 0} {
            echo 'null filename in redirection'
        } {
            echo 'too many files in redirection: ' $*
        }
        return 1
    }
    result $*
...
fn-%eval-print        = $&noreturn @ { echo $*; $* } # -x
fn-%noeval-noprint    = { }                                   # -n
fn-%noeval-print      = @ { echo $* }                # -n -x
